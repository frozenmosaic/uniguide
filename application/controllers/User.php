...
class User extends Application
{
    ...
    public function addUser($params = null, $password = null) {
        if (!empty($params) && !empty($password)) {
            ...
            if ($this->db->insert($this->_table)) {
          		return Helper::process(1, $params); 
                // // sends confirmation email
                // $objEmail = new Email();
                // if ($objEmail->process(1, array('email' => $params['email'], 'first_name' => $params['first_name'], 'last_name' => $params['last_name'], 'password' => $password, 'hash' => $params['hash'],))) {
                //     return true;
                // }
                // return false;
            }
    ...
    public function getByEmail($email = null) {
        if (!empty($email)) {
            $sql = "SELECT `id` FROM `{$this->_table}` WHERE `email` = '".$this->db->escape($email)."' AND `active` = 1";
            return $this->db->fetchOne($sql);
        }
    }
