	...
	if ($objValid->isValid()) {
		if ($objCatalogue->updateProduct($objValid->_post, $id)) {
			...
			if ($objUpload->upload(CATALOGUE_PATH)) {
				...
				$url = Url::getCurrentUrl(array('action', 'id')).'&action=edited-no-upload';
				$url = substr_replace($url, "/start/admin/", 0, 7);
				Helper::redirect($url);
				...
				<select name="category" id="category" class="sel">
					...
					<?php if (!empty($categories)) { ?>
							<?php foreach ($categories as $cat) { ?>
								<option value="<?php echo $cat['id'];?>" <?php echo $objForm->stickySelect('category', $cat['id'], $product['category']); ?>>
