...
class Catalogue extends Application {
	...
	public function addCategory($name = null) {
		if (!empty($name)) {
			$sql = "INSERT INTO `{$this->_table}` (`name`) VALUES ('".$this->db->escape($name)."')";
			return $this->db->query($sql);
		}
	}

	public function duplicateCategory($id = null, $name = null) {
		if (!empty($name)) {
	...
	public function updateCategory($name = null, $id = null) {
		if (!empty($name) && !empty($id)) {
			$sql = "UPDATE `{$this->_table}` SET `name` = '".$this->db->escape($name)."' WHERE `id` = '".$this->db->escape($id)."'";
			return $this->db->query($sql);
		}
		return false;
	}
